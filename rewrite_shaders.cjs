...
let content = fs.readFileSync('src/shaders.js', 'utf8');

// The halftone section is duplicated / broken.
// We'll replace from 'export const halftoneFragment = ' down to '// 7. Film Burn Fragment'
const halftoneNew = `export const halftoneFragment = \`#version 300 es
...
content = content.replace(/export const halftoneFragment = `#version 300 es[\s\S]*?\/\/ 7\. Film Burn Fragment/m, halftoneNew);


const photocopyNew = `export const photocopyFragment = \`#version 300 es
precision highp float;
in vec2 vTexCoord;
out vec4 fragColor;
uniform sampler2D inputTexture;
...
// 14.`;
content = content.replace(/export const photocopyFragment = `#version 300 es[\s\S]*?\/\/ 14\./m, photocopyNew);


const screenprintNew = `export const screenPrintFragment = \`#version 300 es
precision highp float;
in vec2 vTexCoord;
out vec4 fragColor;
uniform sampler2D inputTexture;
...
// 17.`;
content = content.replace(/export const screenPrintFragment = `#version 300 es[\s\S]*?\/\/ 17\./m, screenprintNew);


const linocutNew = `export const linoCutFragment = \`#version 300 es
precision highp float;
in vec2 vTexCoord;
out vec4 fragColor;
uniform sampler2D inputTexture;
...
// 20.`;
content = content.replace(/export const linoCutFragment = `#version 300 es[\s\S]*?\/\/ 20\./m, linocutNew);


const typewriterNew = `export const typewriterFragment = \`#version 300 es
precision highp float;
in vec2 vTexCoord;
out vec4 fragColor;
uniform sampler2D inputTexture;
...
// 22.`;
content = content.replace(/export const typewriterFragment = `#version 300 es[\s\S]*?\/\/ 22\./m, typewriterNew);

fs.writeFileSync('src/shaders.js', content);
console.log('Done!');
